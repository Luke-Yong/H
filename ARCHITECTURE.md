...
**Cross-navigation location** — `navigator.geolocation` is overridden at `dom-ready` so the page always uses H's native Windows location bridge, even after navigating between routes.
...
H and [Graphify](https://github.com/Graphify-Labs/graphify) share the same core idea: pre-build a knowledge graph so AI agents can answer structural questions with a single query instead of scanning raw files. The differences are in scope and design philosophy:
...
H includes an automated test suite using [Vitest](https://vitest.dev). Tests cover all agent tools, the agent loop (with mocked DeepSeek API), tool schema validation, and API endpoints.
